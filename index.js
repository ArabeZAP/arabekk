	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `SLV @${num.split('@')[0]}\nBEM VINDO (A) AO GP *${mdata.subject}* LEIA AS REGRAS PARA N TOMAR BAN POHA`
				...
				teks = `VAI PELA SOMBRA IRMÃO PQ BOSTA NO SOL SECA kkkk FDP VAI FAZER NEM FALTA GHOST DO CARAI @${num.split('@')[0]}👋`
				let buff = await getBuffer(ppimg)
				client.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '[❗] Este comando só pode ser usado por adms',
					Badmin: '[❗] Este comando só pode ser usado quando vc me dar adm bb kk',
                                        daftarB: `── 「REGISTRE-SE」 ──\nslv mano \nVocê n está registrado no banco de dados Do BOT DO ÁRABE, tem que se cadastrar pra ter acesso ao menu, \n\nComando : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar arab|19`,
				}
			}
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                   ...
                   break
					...
					if (args.length < 1) return reply('1 ativa e 0 deliga')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('pronto man')
						welkom.push(from)
